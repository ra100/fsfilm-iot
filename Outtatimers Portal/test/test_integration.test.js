 ...
 * Web Interface → API Endpoints → ConfigManager → Portal Effect
 */
...
class MockSystem {
  constructor() {
    ...
    this.portalEffect = new MockPortalEffect()
  }
  ...
  simulateCompleteFlow(webMinSat, webMaxSat) {
    ...
    // 4. Portal effect uses the values
    this.portalEffect.updateSaturation(configValues.min, configValues.max)

    return {
      webParams: apiParams,
      configValues: configValues,
      effectUpdated: this.portalEffect.isUpdated()
    }
  }
}
...
function testEndToEndWorkflow() {
  ...
  // 5. Portal effect is updated
  system.portalEffect.updateSaturation(configValues.min, configValues.max)
  ...
  assert(system.portalEffect.isUpdated())
